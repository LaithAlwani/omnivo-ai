      ...
      {open && (
        <>
          <button
            ...
            tabIndex={-1}
          />
          <div className="absolute left-0 top-full z-20 mt-1.5 w-60 rounded-xl border border-line-strong bg-surface p-1.5 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.8)] max-md:fixed max-md:inset-x-3 max-md:top-17 max-md:mt-0 max-md:w-auto">
            {businesses?.map((b) => (
              <Link
                key={b._id}
